package quanlynganhang.DTO;

public class ChucVuDTO {
    private int maChucVu, isAdmin, qLThongKe, qLGiaoDich, qLGuiTietKiem, qLVayVon, qLVayTinDung, biXoa;
    private String tenChucVu, moTa, qLKhachHang, qLNhanVien, qLTKKhachHang,qLTKNhanVien, qLThe;

    public ChucVuDTO() {
    }

    public ChucVuDTO(int maChucVu, int isAdmin, int qLThongKe, int qLGiaoDich, int qLGuiTietKiem, int qLVayVon, int qLVayTinDung, int biXoa, String tenChucVu, String moTa, String qLKhachHang, String qLNhanVien, String qLTKKhachHang, String qLTKNhanVien, String qLThe) {
        this.maChucVu = maChucVu;
        this.isAdmin = isAdmin;
        this.qLThongKe = qLThongKe;
        this.qLGiaoDich = qLGiaoDich;
        this.qLGuiTietKiem = qLGuiTietKiem;
        this.qLVayVon = qLVayVon;
        this.qLVayTinDung = qLVayTinDung;
        this.biXoa = biXoa;
        this.tenChucVu = tenChucVu;
        this.moTa = moTa;
        this.qLKhachHang = qLKhachHang;
        this.qLNhanVien = qLNhanVien;
        this.qLTKKhachHang = qLTKKhachHang;
        this.qLTKNhanVien = qLTKNhanVien;
        this.qLThe = qLThe;
    }

    public int getMaChucVu() {
        return maChucVu;
    }

    public void setMaChucVu(int maChucVu) {
        this.maChucVu = maChucVu;
    }

    public int getIsAdmin() {
        return isAdmin;
    }

    public void setIsAdmin(int isAdmin) {
        this.isAdmin = isAdmin;
    }

    public int getqLThongKe() {
        return qLThongKe;
    }

    public void setqLThongKe(int qLThongKe) {
        this.qLThongKe = qLThongKe;
    }

    public int getqLGiaoDich() {
        return qLGiaoDich;
    }

    public void setqLGiaoDich(int qLGiaoDich) {
        this.qLGiaoDich = qLGiaoDich;
    }

    public int getqLGuiTietKiem() {
        return qLGuiTietKiem;
    }

    public void setqLGuiTietKiem(int qLGuiTietKiem) {
        this.qLGuiTietKiem = qLGuiTietKiem;
    }

    public int getqLVayVon() {
        return qLVayVon;
    }

    public void setqLVayVon(int qLVayVon) {
        this.qLVayVon = qLVayVon;
    }

    public int getqLVayTinDung() {
        return qLVayTinDung;
    }

    public void setqLVayTinDung(int qLVayTinDung) {
        this.qLVayTinDung = qLVayTinDung;
    }

    public int getBiXoa() {
        return biXoa;
    }

    public void setBiXoa(int biXoa) {
        this.biXoa = biXoa;
    }

    public String getTenChucVu() {
        return tenChucVu;
    }

    public void setTenChucVu(String tenChucVu) {
        this.tenChucVu = tenChucVu;
    }

    public String getMoTa() {
        return moTa;
    }

    public void setMoTa(String moTa) {
        this.moTa = moTa;
    }

    public String getqLKhachHang() {
        return qLKhachHang;
    }

    public void setqLKhachHang(String qLKhachHang) {
        this.qLKhachHang = qLKhachHang;
    }

    public String getqLNhanVien() {
        return qLNhanVien;
    }

    public void setqLNhanVien(String qLNhanVien) {
        this.qLNhanVien = qLNhanVien;
    }

    public String getqLTKKhachHang() {
        return qLTKKhachHang;
    }

    public void setqLTKKhachHang(String qLTKKhachHang) {
        this.qLTKKhachHang = qLTKKhachHang;
    }

    public String getqLTKNhanVien() {
        return qLTKNhanVien;
    }

    public void setqLTKNhanVien(String qLTKNhanVien) {
        this.qLTKNhanVien = qLTKNhanVien;
    }

    public String getqLThe() {
        return qLThe;
    }

    public void setqLThe(String qLThe) {
        this.qLThe = qLThe;
    }

    
}
